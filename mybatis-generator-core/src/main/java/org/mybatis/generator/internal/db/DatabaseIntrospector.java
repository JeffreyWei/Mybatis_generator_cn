...
public class DatabaseIntrospector {
	...
	private Map<ActualTableName, List<IntrospectedColumn>> getColumns(
		...
		while (rs.next()) {
			...
			//oracle注释需要手动填充
			if (databaseMetaData.toString().indexOf("oracle") > -1) {
				introspectedColumn.setRemarks(OracleColComments.getComment(databaseMetaData.getConnection(), localTableName, rs.getString("COLUMN_NAME"))); //$NON-NLS-1$
			} else {
	 ...
	 * @return
	 */
	private String getRemmark(String localTableName, String column_name) {
		String remark = "";
		String sql = "select COMMENTS from user_col_comments where table_name='" + localTableName + "' and column_name='" + column_name + "'";
		Statement stmt;
		ResultSet rs;
		try {
